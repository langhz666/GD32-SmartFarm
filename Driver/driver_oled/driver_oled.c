...
void OLED_I2C_SendByte(uint8_t Byte)
{
    ...
    /* 循环8次，主机依次发送数据的每一位（高位先行） */
    for (i = 0; i < 8; i++)
    {
        /* 1. 拉低SCL，准备改变SDA电平 */
        IIC_SCL_LOW();
        IIC_DELAY_US(2);  // 时序延时，保证稳定
		
        /* 2. 使用掩码取出Byte的指定一位，写入SDA线 */
        /* 两个!的作用：将非0值转为1，0值保持0 */
        ...
        IIC_DELAY_US(2);  // 确保SDA电平稳定后再操作SCL
		
        /* 3. 释放SCL（拉高），从机在SCL高电平期间读取SDA */
        IIC_SCL_HIGH();
        IIC_DELAY_US(2);  // 延长高电平时间，保证从机足够读取
